...
public class Message {
	...
	public void parse(CommandSender sender, String message) {
		parse(sender, message, DEFAULT_COLOR);
	}

	public void parseNo(CommandSender sender, String message) {
		parse(sender, message, COLOR_NO);
	}

	// Formats player messages with colors
	public void parse(CommandSender sender, String message, ChatColor color) {
		sender.sendMessage(process(message, color));
	}
	...
	public void sendNo(CommandSender sender, String message) {
		sender.sendMessage(COLOR_NO + message);
	}
	...
	public void languageUp(Player player, int raceID, int increase, int languageSkill) {
		player.sendMessage(ChatColor.GOLD + "Language: " + LANGUAGE_NAME_COLOR + plugin.config.NPC_LANGUAGE_NAMES.get(raceID) + ChatColor.GREEN + " skill " + ChatColor.WHITE + "+" + increase + ChatColor.GRAY + " (" + languageSkill + "/100)");
		if (languageSkill == 100)
			player.sendMessage(ChatColor.YELLOW + "Congratulations!  " + DEFAULT_COLOR + "You are now fluent in " + LANGUAGE_NAME_COLOR + plugin.config.NPC_LANGUAGE_NAMES.get(raceID) + "!");
	}
