...
public class LotQueries {
	...
	public ArrayList<Lot> getLots(int id_Player) {
		...
		try {
			...
			String selectQuery = "SELECT lotId FROM Lots WHERE ID_Player = " + id_Player;

			preparedStatement = conn.prepareStatement(selectQuery);
			rs = preparedStatement.executeQuery();

			while (rs.next()) {
				playerLots.add(plugin.lots.get(rs.getInt("lotId"))); // We already have the lot info in memory; save the DB some work
			}
	...
	public ArrayList<String> getLotPermissions(int id_Player) {
		...
		try {
			conn = rageDB.getConnection();
			String selectQuery = "SELECT p.Name as Name FROM LotPermissions lp " + "INNER JOIN Players p ON p.ID_Player = lp.ID_Player_Builder " + "WHERE lp.ID_Player_Owner = " + id_Player;
	...
	public HashMap<String, Lot> loadLots() {
		...
		try {
			conn = rageDB.getConnection();
			preparedStatement = conn.prepareStatement("SELECT l.lotId, l.Category, l.Number, IFNULL(p.Name, '') as Owner, l.XCoord, l.ZCoord, " + "l.Width, l.Height " + "FROM Lots l " + "LEFT JOIN Players p ON l.ID_Player = p.ID_Player");
			...
			while (rs.next()) {
				...
				currentLot.lotId = rs.getInt("lotId");
				...
				currentLot.region = new Region2D(world, ((rs.getInt("ZCoord") - 1) * mult) + plugin.config.Lot_X_OFFSET, ((rs.getInt("XCoord") - 1) * mult * -1) + plugin.config.Lot_Z_OFFSET, ((rs.getInt("ZCoord") - 1) * mult) + plugin.config.Lot_X_OFFSET + (rs.getInt("Height") * mult), ((rs.getInt("XCoord") - 1) * mult * -1) + plugin.config.Lot_Z_OFFSET - (rs.getInt("Width") * mult));
	...
	public void lotClaim(PlayerData playerData, Lot lot) {
		...
		try {
			...
			preparedStatement = conn.prepareStatement("UPDATE Lots SET ID_Player = " + playerData.id_Player + ", DateClaimed = NOW() WHERE lotId = " + lot.lotId);
	...
	public void lotUnclaim(Lot lot) {
		...
		try {
			conn = rageDB.getConnection();
			// Update the Lots table to assign the owner
			preparedStatement = conn.prepareStatement("UPDATE Lots SET ID_Player = NULL, DateClaimed = NULL WHERE lotId = " + lot.lotId);
			preparedStatement.executeUpdate();
		} catch (SQLException e) {
	...
	public void allow(int id_Player_Owner, int id_Player_Builder) {
		...
		try {
			conn = rageDB.getConnection();
			preparedStatement = conn.prepareStatement("INSERT INTO LotPermissions (ID_Player_Owner, ID_Player_Builder) VALUES (" + id_Player_Owner + ", " + id_Player_Builder + ")");
			preparedStatement.executeUpdate();
		} catch (SQLException e) {
	...
	public void disallow(int id_Player_Owner, int id_Player_Builder) {
		...
		try {
			...
			if (id_Player_Builder == 0) {
				preparedStatement = conn.prepareStatement("DELETE FROM LotPermissions WHERE ID_Player_Owner = " + id_Player_Owner);
			} else {
				preparedStatement = conn.prepareStatement("DELETE FROM LotPermissions WHERE ID_Player_Owner = " + id_Player_Owner + " AND ID_Player_Builder = " + id_Player_Builder);
			}
