...
public class InventoryUtilities {
	public static boolean checkFreeSpace(ItemStack[] contents, ItemStack item, int spaceNeeded) {
		...
		for (int i = 0; i < j; i++) {
			...
			if (curItem == null) {
				...
				if ((curItem.getType() != itemType) || ((curItem.getDurability() != durability) && (curItem.getDurability() != -1))) {
					continue;
				}
	...
	public static boolean checkFreeSpace(Inventory inv, ItemStack item, int spaceNeeded) {
		return checkFreeSpace(inv.getContents(), item, spaceNeeded);
	}

	public static void addItemToInventory(Inventory inv, ItemStack item, int amount) {
