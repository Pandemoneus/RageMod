...
public class NPCTown extends Town {
	...
	public void createRegion(double x1, double z1, double x2, double z2) {
		region = new Region3D(world, x1, z1, x2, z2);
		this.centerPoint = new Location(this.world, GeneralUtilities.between(x1, x2), 65, GeneralUtilities.between(z1, z2));
	}

	public void createRegion(String coords) {
		region = new Region2D(world, coords);
		this.centerPoint = new Location(this.world, GeneralUtilities.between(region.nwCorner.getX(), region.seCorner.getX()), 65, GeneralUtilities.between(region.nwCorner.getZ(), region.seCorner.getZ()));
	}
