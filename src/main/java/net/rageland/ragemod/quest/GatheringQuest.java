...
public class GatheringQuest extends QuestOld {
	private ItemStack itemToGather;

	public GatheringQuest(QuestData questData, RewardData rewardData, Flags flags, ItemStack itemToGather) {
	...
	public void end(Player player, PlayerData playerData) {
		...
		if (InventoryUtilities.checkHasEnoughOfItem(player.getInventory().getContents(), itemToGather, questData.getObjectiveCounter()) && InventoryUtilities.checkFreeSpace(player.getInventory(), rewardData.getItem(), rewardData.getAmountOfItems() - additionalFreespace)) {
			...
			if (rewardData.getAmountOfItems() > 0) {
				player.sendMessage(ChatColor.DARK_GREEN + Integer.toString(rewardData.getAmountOfItems()) + ChatColor.GOLD + rewardData.getItem().getType().toString());
				InventoryUtilities.addItemToInventory(player.getInventory(), rewardData.getItem(), rewardData.getAmountOfItems());
			}

			if (rewardData.getCoins() > 0.0D) {
				player.sendMessage(ChatColor.GOLD + " " + RageMod.economy.format(rewardData.getCoins()));
				RageMod.economy.depositPlayer(player.getName(), rewardData.getCoins());
			}
