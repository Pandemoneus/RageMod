...
public class NPCHandler {
	...
	public NPCHandler(RageMod plugin) {
		this.plugin = plugin;
		random = new Random();
	...
	public NPC getRageEntity(int id) {
		return (NPC) activeNPCs.get(id).getEntity();
	}
	...
	public int getNPCIdFromEntity(org.bukkit.entity.Entity e) {
		if ((e instanceof HumanEntity)) {
			for (int i : activeNPCs.keySet()) {
				if (((NPC) activeNPCs.get(i).getEntity()).getBukkitEntity().getEntityId() == ((HumanEntity) e).getEntityId()) {
					return i;
				}
			}
		}
	...
	public static NPC getNPCFromEntity(org.bukkit.entity.Entity e) {
		if ((e instanceof HumanEntity)) {
			for (int i : RageMod.getInstance().npcManager.activeNPCs.keySet()) {
				if (((NPC) RageMod.getInstance().npcManager.activeNPCs.get(i).getEntity()).getBukkitEntity().getEntityId() == ((HumanEntity) e).getEntityId()) {
					return (NPC) RageMod.getInstance().npcManager.activeNPCs.get(i).getEntity();
				}
			}
		}
		return null;
	...
	private class SL extends ServerListener {
		...
		public void onPluginDisable(PluginDisableEvent event) {
			if (event.getPlugin() == NPCHandler.this.plugin) {
				NPCHandler.this.despawnAll(false);
				NPCHandler.this.plugin.getServer().getScheduler().cancelTask(NPCHandler.this.taskid);
			}
		}
	}
	...
	public boolean contains(NPC RageEntity) {
		for (NPCInstance instance : activeNPCs.values()) {
	...
	public NPCInstance spawn(NPCLocation location, NPCData npc) throws Exception {
		...
		instance = plugin.database.npcQueries.createInstance(npc.id_NPC, location.getID(), this.generateTTL(), 0, NPCType.SPEECH);
