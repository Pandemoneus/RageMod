...
/**
 * 
 * @author TheIcarusKid Provides a layer between NPCEntity and the custom entity
...
public class NPC extends NPCEntity {
	...
	public NPC(NPCInstance instance) {
	...
	public void facePlayer(Player player) {
		float yaw = (float) ((Math.atan2(instance.getLocation().getX() - player.getLocation().getX(), player.getLocation().getZ() - instance.getLocation().getZ()) * 180) / Math.PI);
	...
	public class SpeechTask extends TimerTask {
		...
		public SpeechTask(NPC npcEntity, Timer timer, SpeechData speechData) {
		...
		public void run() {
			if (RageMod.getInstance().npcManager.contains(npcEntity) && speechData.getInterval() > 0) {
				Player[] players = NPC.this.plugin.getServer().getOnlinePlayers();
				...
				if (RageMod.getInstance().npcManager.contains(this.npcEntity))
					this.timer.schedule(new SpeechTask(NPC.this, this.timer, speechData), speechData.getInterval());
			}
		}
	}
