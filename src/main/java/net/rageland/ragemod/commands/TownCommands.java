...
public class TownCommands {
	...
	public void create(Player player, String townName) {
		...
		if (!RageMod.economy.has(player.getName(), cost)) {
			plugin.message.parseNo(player, "You need at least " + RageMod.economy.format(cost) + " to create a " + plugin.config.townLevels.get(1).name + ".");
			return;
		}
		...
		if (!townName.equals("")) {
			...
			RageMod.economy.withdrawPlayer(player.getName(), cost);

			plugin.message.parse(player, "Congratulations, you are the new mayor of " + playerTown.getCodedName() + "!");
		} else {
			plugin.message.parse(player, "This location is valid for a new town - to create one for " + RageMod.economy.format(cost) + ", type '/town create <town_name>'");
		}
	}
	...
	public void deposit(Player player, String amountString) {
		...
		if (!RageMod.economy.has(player.getName(), amount)) {
			plugin.message.parseNo(player, "You only have " + RageMod.economy.format(RageMod.economy.getBalance(player.getName())) + ".");
			return;
		}

		// Subtract the amount from the player's balance
		RageMod.economy.withdrawPlayer(player.getName(), amount);
		...
		plugin.message.parse(player, "Deposited " + RageMod.economy.format(amount) + " into town treasury.");
	}
	...
	public void evict(Player player, String targetPlayerName) {
		...
		targetPlayerData.logonMessageQueue += "You have been evicted from " + plugin.towns.get(playerData.townName).getCodedName() + " by " + playerData.getCodedName() + ".<br>";
	...
	public void info(Player player, String townName) {
		...
		if (town instanceof PlayerTown) {
			...
			if (playerData.townName.equalsIgnoreCase(townName)) {
				plugin.message.parse(player, "   Total Balance: " + RageMod.economy.format(playerTown.treasuryBalance));
				plugin.message.parse(player, "   Minimum Balance:  " + RageMod.economy.format(playerTown.minimumBalance));
				plugin.message.parse(player, "   Your Balance:  " + RageMod.economy.format(playerData.treasuryBalance));
			}
	...
	public void minimum(Player player, String amountString) {
		...
		if (amount < playerTown.townLevel.minimumBalance) {
			plugin.message.parseNo(player, "The lowest minimum balance allowed for a " + playerTown.townLevel.name + " is " + RageMod.economy.format(playerTown.townLevel.minimumBalance) + ".");
			return;
		}
		...
		plugin.message.parse(player, "Your town's treasury minimum balance is now " + RageMod.economy.format(amount) + ".");
	}
	...
	public void upgrade(Player player, boolean isConfirmed) {
		...
		if (playerTown.treasuryBalance < targetLevel.initialCost) {
			plugin.message.parseNo(player, "You need at least " + RageMod.economy.format(targetLevel.initialCost) + " in your treasury to upgrade your town to a " + targetLevel.name + ".");
			return;
		}
		...
		if (isConfirmed) {
			...
			plugin.message.parse(player, " " + RageMod.economy.format(targetLevel.initialCost) + " has been deducted from the town treasury.");
		} else {
			plugin.message.parse(player, "Your town is ready to be upgraded to a " + targetLevel.name + " for " + RageMod.economy.format(targetLevel.initialCost) + "; type '/town upgrade confirm' to complete the upgrade.");
		}
	...
	public void withdrawl(Player player, String amountString) {
		...
		if (playerData.treasuryBalance < amount) {
			plugin.message.parseNo(player, "You only have " + RageMod.economy.format(playerData.treasuryBalance) + " in the treasury.");
			return;
		}
		...
		RageMod.economy.depositPlayer(player.getName(), amount);
