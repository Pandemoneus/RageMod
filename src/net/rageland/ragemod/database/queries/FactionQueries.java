...
public class FactionQueries {
	...
	public HashMap<Integer, Integer> getFactionPopulations() {
		...
		try {
			...
			preparedStatement = conn.prepareStatement("SELECT p.ID_Faction, COUNT(*) as Population FROM Factions f " + "INNER JOIN Players p ON p.ID_Faction = f.ID_Faction " + "WHERE DATE_SUB(CURDATE(),INTERVAL 30 DAY) <= p.LastLogin " + "GROUP BY f.ID_Faction");
