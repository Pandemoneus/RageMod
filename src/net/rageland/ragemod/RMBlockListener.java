...
public class RMBlockListener extends BlockListener {
		...
		// *** ZONE B (War Zone) ***
			...
			if (block.getType() == Material.BED_BLOCK) {
				...
				if (playerTown != null && playerTown.getName().equals(playerData.townName)) {
					...
					// Make sure the location is not too close to another
					// player's spawn
	...
	// Generic permission edit handler that handles multiple types of block
	// editing
	private boolean canEditBlock(BlockEvent event, Player player) {
		...
		if (town instanceof NPCTown) {
			...
			if (!RageMod.permission.has(player, "ragemod.build.npctown") && !playerData.npcTownName.equalsIgnoreCase(town.getName()) && !npcTown.hasPermission(playerData.name)) {
		...
		// *** ZONE A (Neutral Zone) ***
			...
			if (plugin.zones.isInCapitol(location)) {
				// If the player is below y=22, make sure they have permission
				// to mine below city (don't check lots)
				if (location.getBlockY() < 22 && !RageMod.permission.has(player, "ragemod.mine.capitol")) {
					plugin.message.sendNo(player, "You don't have permission to mine below the city.");
					return false;
				}
				// See if the player is inside a lot, and if they own it
				else if (!playerData.isInsideOwnLot(location) && RageMod.permission.has(player, "ragemod.build.anylot")) {
					Lot lot = plugin.lots.findCurrentLot(location);

					if (lot == null) {
						if (RageMod.permission.has(player, "ragemod.build.capitol") || playerData.permits.capitol == true)
							return true;
						...
						if (lot.owner.equals(""))
							...
							// Check to see if the player has permission to
							// build in this lot
		...
		// *** ZONE B (War Zone) ***
			...
			if (town != null && town instanceof PlayerTown) {
				...
				if (!playerTown.getName().equals(playerData.townName) && !RageMod.permission.has(player, "ragemod.build.anytown")) {
		...
		// *** TRAVEL ZONE ***
		else if (plugin.zones.isInTravelZone(location) && !RageMod.permission.has(player, "ragemod.build.travelzone")) {
