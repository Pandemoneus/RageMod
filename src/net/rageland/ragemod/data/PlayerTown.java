...
public class PlayerTown {
	...
	public void createRegions() {
		region = new Region3D(world, centerPoint.getBlockX() - (townLevel.size / 2), centerPoint.getBlockY(), centerPoint.getBlockZ() + (townLevel.size / 2), centerPoint.getBlockX() + (townLevel.size / 2), centerPoint.getBlockY(), centerPoint.getBlockZ() - (townLevel.size / 2));
		sanctumFloor = new Region3D(world, centerPoint.getX() - 10, centerPoint.getY() - 1, centerPoint.getZ() + 9, centerPoint.getX() + 9, centerPoint.getY() - 3, centerPoint.getZ() - 10);
		sanctumRoom = new Region3D(world, centerPoint.getX() - 10, centerPoint.getY() + 4, centerPoint.getZ() + 9, centerPoint.getX() + 9, centerPoint.getY(), centerPoint.getZ() - 10);
	...
	private void placeOverlay(int x, int z) {
		for (int y = 127; y >= 1; y--) {
			...
			if (upperType == 0 && lowerType != 0) {
				...
				else if (Material.getMaterial(lowerType) != Material.LEAVES && Material.getMaterial(lowerType) != Material.TORCH) {
		...
		if (rawEvent instanceof BlockBreakEvent) {
			...
			if (playerData.townName.equals(this.name)) {
				...
				if (playerData.treasuryBlocks < 1) {
					...
					playerData.update();
					plugin.message.parse(player, "You now have " + playerData.treasuryBlocks + " block" + (playerData.treasuryBlocks == 1 ? "" : "s") + " deposited into the treasury.", ChatColor.GOLD);
					return true;
				}
			}
			...
			if (playerData.townName.equals(this.name)) {
				...
				if (playerData.treasuryBlocks + 1 > this.townLevel.treasuryLevel) {
					plugin.message.parse(player, "Your town only allows " + this.townLevel.treasuryLevel + " treasury block" + (this.townLevel.treasuryLevel == 1 ? "" : "s") + " per resident.");
					return false;
				} else {
					playerData.treasuryBlocks++;
					playerData.update();
	...
	private void buildSanctumFloor() {
		Build.sanctumFloor(plugin, this.world, (int) this.centerPoint.getX() - 10, (int) this.centerPoint.getY() - 1, (int) this.centerPoint.getZ() - 10, this.townLevel.level, this.id_Faction);
	}
