...
public class Build {
	public static void sanctumFloor(RageMod plugin, World world, int cornerX, int cornerY, int cornerZ, int level, int id_Faction) {
		...
		// Store a list of blocks to light with fire to activate portals (this
		...
		for (int x = 0; x < 20; x++) {
			for (int z = 0; z < 20; z++) {
				...
				case 'w':
					...
					currentBlock.setData((byte) Byte.valueOf(String.valueOf(Faction.getWoolColor(id_Faction))));
					break;
				case '-':
					currentBlock.setType(Material.OBSIDIAN);
		...
		for (Block portalBlock : portalBlocks) {
			...
			break; // only one block is needed, trying to light more of them
					// causes the portal to break
		}
	}
	...
	public static void travelNode(Location location, int size, int id_Faction) {
		...
		int locY = (int) (location.getY());
		int locZ = (int) (location.getZ());
		int offset = (int) (size / 16); // 1/8 scale for nether, divided by 2
