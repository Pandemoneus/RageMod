...
public class LotCommands {
	...
	/*public void onLotCommand(Player player, PlayerData playerData, String[] split) {
		if (split.length < 2 || split.length > 4) {
			...
			if (RageMod.permission.has(player, "ragemod.lot.assign"))
			...
			if (RageMod.permission.has(player, "ragemod.lot.evict"))
	...
	public void claim(Player player, String lotCode) {
		...
		for (Lot ownedLot : playerData.lots) {
			if (ownedLot.category == lot.category && (lot.category == LotType.WARRENS || lot.category == LotType.MARKET)) {
				plugin.message.sendNo(player, "You can only own one " + lot.getCategoryName() + " lot at a time.");
				return;
			} else if ((lot.category == LotType.COAL || lot.category == LotType.IRON || lot.category == LotType.GOLD || lot.category == LotType.DIAMOND) && (ownedLot.category == LotType.COAL || ownedLot.category == LotType.IRON || ownedLot.category == LotType.GOLD || ownedLot.category == LotType.DIAMOND)) {
		...
		if (lot.category == LotType.MARKET) {
	...
	public void list(Player player) {
		...
		for (Lot lot : playerData.lots) {
			plugin.message.parse(player, "   " + lot.getLotCode() + " (" + lot.getCategoryName() + ")  " + "x: " + (int) lot.region.nwCorner.getX() + "  z: " + (int) lot.region.nwCorner.getZ());
		}
