...
public class FactionCommands {
	...
	public void join(Player player, String factionName) {
		...
		if (factionName.equals("")) {
			...
			for (int faction : populations.keySet()) {
				plugin.message.parse(player, "   " + plugin.factions.getCodedName(faction) + ": " + RageMod.economy.format(populations.get(faction)));
			}
			return;
		}
		...
		if (populations.get(id_Faction) != null && !RageMod.economy.has(playerName, populations.get(id_Faction))) {
			plugin.message.parseNo(player, "You need at least " + RageMod.economy.format(populations.get(id_Faction)) + " to join the " + plugin.factions.getCodedName(id_Faction) + " faction.");
			return;
		}

		// Subtract from player balance
		RageMod.economy.withdrawPlayer(playerName, populations.get(id_Faction));
