...
public class Commands {
	...
	public void home(Player player, String targetPlayerName) {
		...
		if (targetPlayerData.id_Player != playerData.id_Player && !RageMod.permission.has(player, "ragemod.referee.inspectspawn")) {
		...
		if (playerData.home_LastUsed != null) {
			...
			if (secondsSinceLastUse < plugin.config.Cooldown_Home) {
				plugin.message.parseNo(player, "Spell /home is not ready yet (" + GeneralUtilities.formatCooldown(plugin.config.Cooldown_Home - secondsSinceLastUse) + " left)");
				return;
			}
		}
	...
	public void spawn(Player player, String targetPlayerName) {
		...
		if (playerData.spawn_LastUsed != null) {
			...
			if (secondsSinceLastUse < plugin.config.Cooldown_Spawn) {
				plugin.message.parseNo(player, "Spell /spawn is not ready yet (" + GeneralUtilities.formatCooldown(plugin.config.Cooldown_Spawn - secondsSinceLastUse) + " left)");
				return;
			}
		}
	...
	public void affinity(Player player) {
		...
		for (float affinity : playerData.getAffinities().values()) {
			int affinityCode = GeneralUtilities.getAffinityCode(affinity);
