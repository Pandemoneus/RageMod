...
public class NPCTownCommands {
	...
	/*public void onNPCTownCommand(Player player, PlayerData playerData, String[] split) {
		if (split.length < 2 || split.length > 5) {
			...
			if (RageMod.permission.has(player, "ragemod.npctown"))
	...
	private void create(Player player, String name, String raceString, String coords) {
		...
		try {
			...
			int id_NPCTown = plugin.database.npcTownQueries.create(player, name, Integer.parseInt(split[0]), Integer.parseInt(split[1]), Integer.parseInt(split[2]), Integer.parseInt(split[3]), 1, id_Race);
	...
	public void info(Player player, String townName) {
		...
		if (town instanceof PlayerTown) {
			...
			for (NPCData npc : plugin.npcManager.getAllResidents(town.getID()))
				residents += ", #" + npc.id_NPC + ": " + npc.getCodedName() + " (" + (npc.isMale ? 'M' : 'F') + ", " + plugin.config.NPC_AFFINITY_NAMES.get(npc.defaultAffinityCode) + ")";
			if (!residents.equals(""))
				plugin.message.parse(player, "   Residents: " + residents.substring(2));

			if (RageMod.permission.has(player, "ragemod.npctown") || playerData.npcTownName.equalsIgnoreCase(town.getName())) {
				for (NPCLocation location : npcTown.getNPCLocations()) {
					plugin.message.parse(player, "   Loc. #" + location.getID() + " (" + (int) location.getX() + ", " + (int) location.getY() + ", " + (int) location.getZ() + ")" + " " + (location.isActivated() ? location.getInstance().getCodedName() : ""));
				}
			}
		}
	}
	...
	public void list(Player player, String factionName) {
		...
		for (NPCTown town : npcTowns) {
			plugin.message.parse(player, "   " + town.getCodedName() + " " + ChatColor.YELLOW + town.getQuadrant().toString() + " (" + town.townLevel.name + ") " + town.getNPCLocations().size() + ChatColor.WHITE + " locations");
		}
		return;
