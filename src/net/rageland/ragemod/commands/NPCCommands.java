...
public class NPCCommands {
	...
	private void createQuestNPC(Player player, String name, String questNpcType, String questId) {
		...
		try {
			...
			/*
			 * npc.isQuestNPC = true; npc.quest =
			 * plugin.questManager.quests.get(questId);
			 * 
	...
	private void spawn(Player player, String id_NPCLocation, String npcString) {
		...
		try {
			...
			instance = plugin.database.npcQueries.createInstance(npc.id_NPC, location.getID(), 30, playerData.id_Player, NPCType.SPEECH);
	...
	private void listloc(Player player) {
		...
		for (NPCLocation location : plugin.npcManager.getAllLocations()) {
			if (location.getTown() == null)
				plugin.message.parse(player, " " + location.getID() + ": " + plugin.zones.getName(location.getZone()) + " (" + location.getQuadrant().toString() + ") " + (location.isActivated() ? ChatColor.GOLD + "ACTIVE" : ""));
		}
	}
		...
		if (instances.size() == 0)
			...
			for (NPCInstance instance : plugin.npcManager.getAllInstances()) {
				...
				if (town != null)
					...
					locationStr = plugin.zones.getName(instance.getLocation().getZone()) + " (" + instance.getLocation().getQuadrant().toString() + ")";
